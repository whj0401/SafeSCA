...
gitCloneURLS= currentPath + "/keen_oss_github_url.txt"             # Please change to the correct file (the "sample" file contains only 10 git-clone urls)
...
ctagsPath    = "/export/d2/hwangdz/data_for_BCA_BSA/ast_parser/ctags_install/bin/ctags"             # Ctags binary path (please specify your own ctags path)
...
def hashing(repoPath):
    ...
    possible = (".c", ".cc", ".cpp", ".cxx", ".h", ".hxx", ".hpp", ".H", ".HPP")
                ...
                try:
                    ...
                    for i in allFuncs:
                        ...
                        if i != '' and len(elemList) >= 8 and func.fullmatch(elemList[3]):
                            ...
                            normalized_funcBody = normalize(funcBody)
                            funcHash = computeTlsh(normalized_funcBody)
                            funcRange = (funcStartLine, funcEndLine)

                            if len(funcHash) == 72 and funcHash.startswith("T1"):
                                funcHash = funcHash[2:]
                            elif funcHash == "TNULL" or funcHash == "" or funcHash == "NULL":
                                continue
                            elif funcHash == '' or funcHash == ' ':
                                continue

                            storedPath = filePath.replace(repoPath, "")
                            if funcHash not in resDict:
                                resDict[funcHash] = []
                            resDict[funcHash].append((storedPath, funcRange[0], funcRange[1], funcProto, funcBody))
...
def indexing(resDict, title, filePath):
    ...
    dump_pkl(resDict, filePath)
    # with open(filePath, 'wb') as of:
    #     pickle.dump(resDict, of, protocol=4)
